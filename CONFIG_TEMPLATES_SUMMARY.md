...
All configuration files have been created in `/home/user/cookiecutter-python-template/foundry_unify/`

### 1. **codecov.yml** - Code Coverage Configuration
- **Purpose**: Configure Codecov for coverage tracking and reporting
- **Cookiecutter Variables**:
  - `Foundry Unify` - Project name in comments
  - `OCR orchestration and layout analysis service for the Foundry RAG pipeline` - Description in comments
  - `foundry_unify` - Module path for coverage tracking
...
- **Cookiecutter Variables**:
  - `foundry_unify` - Package name
  - `Byron Williams` - Copyright holder
  - `https://github.com/ByronWilliamsCPA/Unify` - Download location
...
- **Cookiecutter Variables**:
  - `Foundry Unify` - Site name and title
  - `OCR orchestration and layout analysis service for the Foundry RAG pipeline` - Site description
  - `Byron Williams` - Author and copyright holder
  - `https://Unify.readthedocs.io` - Documentation site URL
  - `https://github.com/ByronWilliamsCPA/Unify` - Repository URL
  - `ByronWilliamsCPA` - GitHub org/user in repo name
  - `foundry_unify` - Project slug for repo name
- **Features**:
...
|----------|------|---------|---------|
| `Foundry Unify` | string | "My Python Project" | codecov.yml, mkdocs.yml |
| `foundry_unify` | auto | Derived from project_name | codecov.yml, renovate.json, REUSE.toml, mkdocs.yml, noxfile.py |
| `OCR orchestration and layout analysis service for the Foundry RAG pipeline` | string | "A short description..." | codecov.yml, mkdocs.yml |
...
| `https://github.com/ByronWilliamsCPA/Unify` | auto | Derived from github info | REUSE.toml, mkdocs.yml |
| `https://Unify.readthedocs.io` | auto | Derived from project_slug | mkdocs.yml |
...
- **ingestion**: PDF/Image Ingestion module (`src/foundry_unify/ingestion/**`)
- **detection**: IQA Detection module (`src/foundry_unify/detection/**`)
- **correction**: Image Correction module (`src/foundry_unify/correction/**`)
- **output**: JSON Output module (`src/foundry_unify/output/**`)
- **utils**: Utilities module (`src/foundry_unify/utils/**`)
